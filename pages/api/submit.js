...
export default async function handler(req, res) {
  ...
  if (!isWhitelisted) {
    ...
    if (ipCount && parseInt(ipCount) >= 5) return res.status(429).json({ error: '🚫 С вашего IP слишком много заявок.' });
  }
...
function getFormTitle(type, department, targetDepartment, leaveType) {
  ...
  if (type === 'transfer') { const n = { 'cid': 'CID', 'fa': 'FA', 'hrt': 'HRT', 'atf': 'ATF', 'af': 'AF', 'ocu': 'OCU', 'dea': 'DEA', 'fna': 'FNA', 'nsb': 'NSB' }; return `🔄 Запрос на перевод в ${n[targetDepartment] || 'Отдел'}`; }
...
function getFormColor(type) {
  const colors = { promotion: 0x4CAF50, transfer: 0x2196F3, report: 0xFF9800, highrank: 0xFF69B4, resignation: 0xDC3545, reinstatement: 0x9C27B0, 'transfer-to-fib': 0x00BCD4, hiring: 0x4CAF50, 'weapon-request': 0xFF5722, leave: 0x00BCD4 };
...
function buildFields(type, department, targetDepartment, data, leaveType, userId, username) {
  ...
  if (type === 'transfer') {
    ...
    if (targetDepartment === 'cid') fields.push(
    ...
    if (targetDepartment === 'fa') fields.push(
...
async function sendBanWordAlert(user, badWords, fullText, type, req) {
  ...
  const embed = {
    title: '🚨 ОБНАРУЖЕН БАНВОРД (без бана)',
    color: 0xFF0000,
    author: { name: user.username, icon_url: `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png` },
    fields: [
    ...
    footer: { text: 'Majestic FIB Forms • Модерация' },
    timestamp: new Date().toISOString()
  };

  try {
    await sendToDiscord(webhookUrl, {
      content: '🚨 **Обнаружен банворд (без бана)!**',
      embeds: [embed],
